...
variable "redshift_max_error" {
  description = "Redshift max error setting which controls amount of acceptable loading errors"
  type        = number
  default     = 1
}

variable "redshift_jsonpaths_bucket" {
  description = "S3 path that holds JSONPaths"
  type        = string
  default     = ""
}

variable "redshift_aws_s3_bucket_name" {
  description = "AWS bucket name where data to load is stored"
  type        = string
}

variable "redshift_aws_s3_folder_monitoring_stage_url" {
  description = "AWS bucket URL of folder monitoring stage - must be within 'redshift_aws_s3_bucket_name' (NOTE: must be set if 'folder_monitoring_enabled' is true)"
  type        = string
  default     = ""
}

variable "redshift_aws_s3_folder_monitoring_transformer_output_stage_url" {
  description = "AWS bucket URL of transformer output stage - must be within 'redshift_aws_s3_bucket_name' (NOTE: must be set if 'folder_monitoring_enabled' is true)"
  type        = string
  default     = ""
}
