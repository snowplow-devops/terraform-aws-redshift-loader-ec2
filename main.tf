...
resource "aws_iam_role" "sts_credentials_role" {
  ...
  description = "Allows the Redshift Loader to access the S3 buckets to perform loading"
  tags        = local.tags
...
locals {
  ...
  config = templatefile("${path.module}/templates/config.json.tmpl", {
    ...
    folder_monitoring_transformer_output = var.redshift_aws_s3_folder_monitoring_transformer_output_stage_url
